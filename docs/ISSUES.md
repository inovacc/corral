...
The `grok` and `kimi` presets compile and self-register (via each
...
`PromptFlag` providers (`grok --single`, `kimi --prompt`) pass
...
the OS command-line length limit (Windows ~32 KB via `CreateProcess`). Grok's and
kimi's stdin handling is **unverified** — an oversized prompt to those two may not
...
## No usage/quota tracking for grok/kimi

`claude`, `codex`, and `agy` implement `UsageReporter` (usage/limit polling). The
`grok` and `kimi` presets do **not** yet — `Monitor`/`checkLimit` returns
no `LimitStatus` for them, so rate-limit awareness is unavailable on those two.
