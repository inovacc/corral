...
**Overall Progress:** foundation complete — 5 provider backends live, single
...
Five providers, each self-registering via `init()` and blank-imported by `all/`:
...
- [ ] `UsageReporter` + `usage.go` for `grok/` and `kimi/` (currently none)
