...
func main() {
	...
	if len(os.Args) > 1 && isLightCommand(os.Args[1]) {
		...
		light.AddCommand(newUsageCmd(), newLaunchCmd(), newServeCmd(), newGenCmd())
	...
	root.AddCommand(newUsageCmd(), newLaunchCmd(), newServeCmd(), newGenCmd())
