...
func isLightCommand(arg string) bool {
	switch arg {
	case "usage", "launch", "serve", "gen", "snippet", "models", "catalog":
		return true
	}
	return false
}
...
func main() {
	...
	if len(os.Args) > 1 && isLightCommand(os.Args[1]) {
		...
		light.AddCommand(newUsageCmd(), newLaunchCmd(), newServeCmd(), newGenCmd(), newSnippetCmd(), newModelsCmd(), newCatalogCmd())
	...
	root.AddCommand(newUsageCmd(), newLaunchCmd(), newServeCmd(), newGenCmd(), newSnippetCmd(), newModelsCmd(), newCatalogCmd())
